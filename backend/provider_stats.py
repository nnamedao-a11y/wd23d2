...
from app.repositories import ProviderStatsRepository
...
class ProviderStatsService:
    def __init__(self, db, bus=None):
        self.db = db
        self.bus = bus
        # Phase 5.3 / C-4: own collection access goes through the
        # repository. `self.db` is preserved for the cross-domain
        # touches (orders / users / staff) which remain on the
        # service side until Phase 5.4 (orders) / 5.5 (identity).
        self._stats_repo = ProviderStatsRepository(db)
    ...
    async def recompute(self, provider_id: str) -> Dict[str, Any]:
        ...
        prev_doc = (await self._stats_repo.get_for_provider(provider_id)) or {}
        ...
        if total == 0:
            ...
            await self._stats_repo.upsert_snapshot(
                provider_id,
                stats=stats,
    ...
    async def get(self, provider_id: str) -> Dict[str, Any]:
        doc = await self._stats_repo.get_for_provider(provider_id)
        if not doc:
            doc = await self.recompute(provider_id)
        return doc

    async def list_all(self, sort_by_score: bool = True) -> List[Dict[str, Any]]:
        # Phase 5.3 / C-4: both branches route through the
        # repository. The `sort_by_score=False` path preserves
        # the legacy unsorted shape via `list_unsorted` (no
        # production caller exercises it today but the public
        # API parameter remains for back-compat).
        if sort_by_score:
            return await self._stats_repo.list_ranked()
        return await self._stats_repo.list_unsorted()
    ...
    async def pick_best_provider(self, candidate_ids: List[str]) -> Optional[str]:
        ...
        if not candidate_ids:
            return None
        docs = await self._stats_repo.find_for_providers(candidate_ids)
