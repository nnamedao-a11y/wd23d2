...
class ShipmentIdentityResolver:
    ...
    async def resolve_vessel(
        ...
        if vf_payload:
            try:
                # Phase 5.5/H (2026-05-20) — canonical home of
                # ``extract_vessels_from_payload`` is the
                # ``vesselfinder_scraper`` module. The legacy
                # ``from server import _vf_extract_vessels`` lazy bridge
                # has been retired together with the alias on the server
                # side; this module now reaches for the canonical
                # no-underscore name directly. Local import preserves
                # the previous lazy-resolution shape (helper is touched
                # only when the caller actually feeds a VF payload).
                from vesselfinder_scraper import (  # type: ignore
                    extract_vessels_from_payload,
                )
                vessels = extract_vessels_from_payload(vf_payload) or []
                if vessels:
