...
async def _ensure_seed(customer_id: str) -> None:
    ...
    Phase 5.5 / D — body moved to
    ``app.services.customers.ensure_customer_seed``. The
    ``from server import _ensure_customer_seed`` lazy bridge has been
    retired in the same wave.
    """
    try:
...
async def get_cabinet_orders():
    ...
    from app.utils.serialization import serialize_doc  # Phase 5.2 / C-1 -- migrated off server.py bridge
    db = _db()
