...
class AutoTransferDetector:
    ...
    async def _audit(self, action: str, shipment_id: str, meta: Dict[str, Any]) -> None:
        try:
            ...
            # SecurityAuditRepository.record_transfer_event. The 4-field
            # shape with FORMATTED resource string ("shipment:{id}") is
            # preserved verbatim — this is shape 5 of 5 in the dual-audit
            # topology (load-bearing per C-1 §1.1).
            from app.repositories import SecurityAuditRepository
            await SecurityAuditRepository(self.db).record_transfer_event({
                "ts": _iso(),
                "action": action,
