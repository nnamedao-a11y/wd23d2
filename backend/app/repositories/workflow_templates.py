...
_DEFAULT_TEMPLATES: list[dict] = [
    {
        "name": "Стандартна (3 кроки)",
        "description": "Базовий цикл для більшості послуг",
        "steps": [
    ...
    {
        "name": "Логістика (повний цикл)",
        "description": "Від отримання до доставки",
        "steps": [
    ...
    {
        "name": "Документи",
        "description": "Оформлення документації",
        "steps": [
...
class WorkflowTemplateRepository:
    ...
    async def seed_default_templates(self) -> list[dict]:
        ...
        cleanup concern.
        """
        # Build fresh seed docs (each call produces new uuids/timestamps)
        seeds = [
            {
                **dict(template),
                "id": _new_template_id(),
