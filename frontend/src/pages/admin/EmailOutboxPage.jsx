  ...
  return (
    <div className={embedded ? '' : 'p-6 max-w-[1280px] mx-auto'}>
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900 flex items-center gap-2">
            <Mail className="w-6 h-6 text-[#635BFF]" /> {t('adm_email_outbox')}
          </h1>
          <p className="text-sm text-zinc-500 mt-1">
            {t('adm_provider')} <span className={`inline-block px-2 py-0.5 rounded-full text-[11px] font-medium ${provider === 'resend' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{provider}</span>
            {provider === 'dry_run' && t('adm3_638c49bee6')}
          </p>
        </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-3 mb-4 flex flex-wrap items-center gap-2">
        ...
        <WhiteSelect value={filterEvent} onChange={(e) => setFilterEvent(e.target.value)} className="px-3 py-2 border border-zinc-200 rounded-lg text-sm bg-white">
        ...
        <table className="w-full text-sm">
          <thead className="bg-zinc-50 text-xs uppercase text-zinc-500">
            <tr>
              ...
              <th className="text-left px-5 py-3 font-medium">{t('adm_recipient')}</th>
              <th className="text-left px-5 py-3 font-medium">{t('subjectLabel')}</th>
              <th className="text-left px-5 py-3 font-medium">{t('adm_time_2')}</th>
              <th />
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && !loading ? (
              <tr><td colSpan={6} className="text-center py-12 text-zinc-400 text-sm">{t('adm_outbox_is_empty_events_have_not_been_triggered_yet')}</td></tr>
            ) : filtered.map((e) => {
