...
const TierBadge = ({ tier }) => {
  const { t } = useLang();
  const config = {
    1: { color: 'bg-blue-500 text-white', label: t('adm_t1_fast') },
    2: { color: 'bg-purple-500 text-white', label: t('adm_t2_mid') },
    3: { color: 'bg-orange-500 text-white', label: t('adm_t3_deep') },
          ...
          <table className="w-full" data-testid="sources-table">
            <thead className="bg-[#F4F4F5] text-left text-sm text-[#71717A]">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">{t('adm_vinlot')}</th>
