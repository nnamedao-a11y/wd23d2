        ...
        <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-lg bg-white rounded-2xl border border-[#E4E4E7] max-h-[90vh] overflow-y-auto" data-testid="deal-modal">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-4">
            ...
            <div>
              ...
              <input type="text" value={formData.vehiclePlaceholder} onChange={(e) => setFormData({...formData, vehiclePlaceholder: e.target.value})} placeholder={t('adm_bmw_x5_2022')} className="input w-full" data-testid="deal-vehicle-input" />
            </div>
            <div>
...
const DealPipelineAdvance = ({ deal, onAdvanced }) => {
  ...
  const doAdvance = async (to) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || t('adm3_3044da855d'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="border-t border-[#E4E4E7] pt-4">
      <div className="text-xs font-semibold uppercase tracking-wider text-[#71717A] mb-2">
        {t('i18n_stage_d975f8')} «{(currentGroup?.labels && currentGroup.labels[lang]) || currentGroup?.label || '—'}»
      </div>
          ...
          {groups.map((g, idx) => {
            ...
            return (
              <span key={g.id}
                ...
                {idx + 1}. {(g.labels && g.labels[lang]) || g.label}
              </span>
            );
          })}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        <span className="text-xs text-[#71717A]">{t('adm_current_stage_3')}</span>
        <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-[#4F46E5] text-white" data-testid="pipeline-current-stage">
          {pretty(current)}
        </span>
      </div>
      {allowed.length === 0 ? (
        <p className="text-xs text-[#71717A]">{t('adm_final_stage_no_transitions')}</p>
      ) : (
