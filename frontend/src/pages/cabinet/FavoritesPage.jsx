  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {added ? (
          <p className="mt-2 text-[11px] text-[#A1A1AA]">{t('adm3_4fe9d8a27f')} {added}</p>
        ) : null}
  ...
  const load = useCallback(
    async (showSpinner = true) => {
      try {
        ...
        if (err?.status === 401) {
          toast.info(t('adm3_7a9a423188'));
          navigate(
            '/cabinet/login?redirect=' + encodeURIComponent(window.location.pathname)
          );
        } else {
          toast.error(err?.message || t('adm3_699295f332'));
        }
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const handleRemove = useCallback(
    async (item) => {
      ...
      try {
        ...
        toast(t('adm3_0c89029e17'), { description: item.title || id });
      } catch (err) {
        toast.error(err?.message || t('adm3_a02aea4dba'));
        // Roll back: reload
        load(false);
      }
    },
    ...
    <div className="space-y-6" data-testid="favorites-page">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[#18181B]">{t('adm3_573104140c')}</h1>
            <p className="text-sm text-[#71717A]">
              {total > 0 ? `${total} ${t('r9_cars_plural')}` : t('adm3_2bfd6ca197')}
            </p>
          </div>
        </div>
