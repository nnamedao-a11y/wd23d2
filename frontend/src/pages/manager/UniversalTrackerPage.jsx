...
const PROVIDER_META = {
  vesselfinder: {
    title: 'VesselFinder',
    descriptionKey: 'utDescVesselRealtime',
  },
  shipsgo: {
    title: 'ShipsGo',
    descriptionKey: 'utDescShipsGo',
  },
  aftership: {
    title: 'AfterShip',
    descriptionKey: 'utDescFallbackTracker',
  },
};
...
const UniversalTrackerPage = () => {
  ...
  const handleSearch = async (e) => {
    e?.preventDefault?.();
    if (!query.trim()) {
      toast.error(t('utEnterIdRequired'));
      return;
    }
    setLoading(true);
    setResult(null);
  ...
  const handleAttach = async (shipmentId, imo, vesselName) => {
    ...
    try {
      ...
      toast.success(`IMO ${imo} ${t('utLinkedToShipment')}`);
      handleSearch();
    } catch (e) {
      toast.error(t('adm_failed_to_link_vessel'));
    }
  };
  ...
  const classificationLabel = (cls) =>
    ({
      vin: 'VIN',
      container: t('adm3_623457effe'),
      imo: t('adm3_63da15c3f0'),
      lot: t('utLotAuction'),
      generic: t('adm3_bbb8739fac'),
      number: t('adm3_1d933d0a6a'),
      empty: '—',
    }[cls] || cls);
  ...
  return (
    <div className="space-y-6" data-testid="manager-tracker-page">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#18181B] flex items-center gap-2">
          <MagnifyingGlass size={26} weight="bold" /> {t('utTitle')}
        </h1>
        ...
        <div className="bg-white border border-[#E4E4E7] rounded-2xl p-5">
            ...
            {Object.entries(providers.providers || {}).map(([key, p]) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-[#71717A] mt-1">{meta.descriptionKey ? t(meta.descriptionKey) : p.purpose}</p>
          ...
          {showConfig && (
            <div className="mt-4 pt-4 border-t border-[#E4E4E7] space-y-3">
              ...
              >
                {savingConfig ? t('utSaving') : t('utSaveKeys')}
              </button>
              <p className="text-xs text-[#71717A]">
                {t('utKeysSaved')}
                {t('r9_in_prefix')}<code>/app/backend/.env</code>.
              </p>
            </div>
          )}
        </div>
      ...
      >
        ...
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder={t('adm_vin_container_imo_lot')}
        ...
        <div className="flex gap-2 mt-3 flex-wrap">
          <span className="text-xs text-[#71717A]">{t('utQuickExamples')}:</span>
          {EXAMPLES.map((ex) => (
            <button
              key={ex.value}
              type="button"
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#E4E4E7] rounded-2xl p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="text-xs text-[#71717A]">{t('recognizedAs')}</div>
          ...
          {result.internal?.shipments?.length > 0 && (
            <div className="bg-white border border-[#E4E4E7] rounded-2xl p-5">
              <h3 className="font-semibold text-[#18181B] mb-3 flex items-center gap-2">
                <Truck size={18} /> {t('utFoundInCrmShipment')}
              </h3>
          ...
          {(result.internal?.vehicles?.length > 0 || result.internal?.deals?.length > 0) && (
            <div className="bg-white border border-[#E4E4E7] rounded-2xl p-5 space-y-3">
              {result.internal.vehicles.length > 0 && (
                <>
                  <h3 className="font-semibold text-[#18181B]">{t('vehicles')}</h3>
              ...
              {result.internal.deals.length > 0 && (
                <>
                  <h3 className="font-semibold text-[#18181B]">{t('deals')}</h3>
                  <div className="space-y-1">
          ...
          {result.external && (
            <div className="bg-white border border-blue-200 rounded-2xl p-5 bg-blue-50/30">
              <h3 className="font-semibold text-[#18181B] flex items-center gap-2 mb-2">
                <Globe size={18} /> {t('utExternalTracker')} — {result.external.source}
              </h3>
              <div className="grid grid-cols-2 gap-3 text-sm">
                {result.external.vesselName && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('vessel')}</div>
                ...
                {result.external.origin && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('portOfDeparture')}</div>
                    <div>{result.external.origin}</div>
                  </div>
                )}
                {result.external.destination && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('portOfArrival')}</div>
                ...
                {result.external.eta && (
                  <div>
                    ...
                    <div className="font-semibold text-blue-600">
                      {new Date(result.external.eta).toLocaleDateString(getLocale())}
                    </div>
                  </div>
                )}
                {result.external.status && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('statusGeneric')}</div>
          ...
          {vesselPosition && (
            <div className="bg-white border border-emerald-200 rounded-2xl p-5 bg-emerald-50/30">
              <h3 className="font-semibold text-[#18181B] flex items-center gap-2 mb-3">
                <Anchor size={18} /> {t('utCurrentVesselPosition')}
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                <div>
                  <div className="text-xs text-[#71717A]">{t('coordinates')}</div>
                ...
                {vesselPosition.speed !== null && vesselPosition.speed !== undefined && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('utSpeedLabel')}</div>
                ...
                {vesselPosition.course !== null && vesselPosition.course !== undefined && (
                  <div>
                    <div className="text-xs text-[#71717A]">{t('heading')}</div>
                    <div>{Math.round(Number(vesselPosition.course))}°</div>
                  </div>
                )}
                <div>
                  <div className="text-xs text-[#71717A]">{t('sourceShort')}</div>
          ...
          {liveShipment && liveShipment.route && liveShipment.origin && liveShipment.destination && (
            <div className="bg-white border border-[#E4E4E7] rounded-2xl p-5">
              <h3 className="font-semibold text-[#18181B] mb-3">{t('routeMap')}</h3>
          ...
          >
            <ArrowClockwise size={14} /> {t('adm_refresh_3')}
          </button>
        </div>
      )}
    </div>
