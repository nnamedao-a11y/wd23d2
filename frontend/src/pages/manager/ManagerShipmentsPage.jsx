...
const ManagerShipmentsPage = () => {
  ...
  const handleAddTracking = async (shipmentId) => {
    if (!trackingNumber.trim()) {
      toast.error(t('enterTrackingNumber'));
      return;
    }
    try {
      ...
      toast.success(t('adm_tracking_added'));
    ...
    >
      ...
      <div>
        ...
        <p className="text-sm text-[#71717A] mt-1">
          {t('managerShipmentsTrackingTitle')}
        </p>
      </div>
      ...
      <div className="space-y-4">
        {shipments.length === 0 ? (
          <div className="bg-white rounded-2xl border border-[#E4E4E7] p-12 text-center">
            ...
            <p className="text-lg font-medium text-[#18181B]">{t('noActiveShipments')}</p>
            <p className="text-sm text-[#71717A]">{t('shipmentsWillAppear')}</p>
          </div>
        ) : (
                  ...
                  {showAddTracking === ship._id ? (
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        placeholder={t('containerOrTrackingPlaceholder')}
