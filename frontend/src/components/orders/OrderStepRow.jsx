...
const STATUS_META = {
  pending:     { icon: Circle,        color: 'text-gray-400',     ring: 'bg-gray-50  border-gray-200',    label: 'Pending' },
  in_progress: { icon: Loader2,       color: 'text-blue-600 animate-spin', ring: 'bg-blue-50 border-blue-200',  label: 'In Progress' },
  done:        { icon: CheckCircle2,  color: 'text-emerald-500',   ring: 'bg-emerald-50 border-emerald-200', label: 'Done' },
  skipped:     { icon: Clock,         color: 'text-amber-500',     ring: 'bg-amber-50 border-amber-200',  label: 'Skipped' },
};
...
export default function OrderStepRow({ step, orderId, readOnly = false, onUpdated }) {
  ...
  const setStatus = async (next) => {
    ...
    try {
      ...
      toast.error(e.response?.data?.detail || t('r9_update_error'));
