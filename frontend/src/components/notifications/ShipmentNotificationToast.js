...
const STATUS_LABEL_KEYS = {
  'deal_created':        'shipStatusDealCreated',
  'contract_signed':     'shipStatusContractSigned',
  'deposit_paid':        'shipStatusDepositPaid',
  'lot_paid':            'shipStatusLotPaid',
  'transport_to_port':   'shipStatusTransportToPort',
  'at_origin_port':      'shipStatusAtOriginPort',
  'loaded_on_vessel':    'shipStatusLoadedOnVessel',
  'in_transit':          'shipStatusInTransit',
  'at_destination_port': 'shipStatusAtDestinationPort',
  'customs':             'shipStatusCustoms',
  'ready_for_pickup':    'shipStatusReadyForPickup',
...
export function ShipmentNotificationToast() {
  ...
  const getTitle = () => {
    switch (type) {
      case 'status':
        return t('adm3_0a14538c5a');
      case 'eta':
        return t('adm3_a50125c75e');
      case 'arrived':
        return t('adm3_f3ef38f781');
      case 'ready':
        return t('adm3_3e28cd64ee');
      default:
        return t('adm3_040972c977');
    }
  };
  ...
  const getMessage = () => {
    switch (type) {
      case 'status':
        const newStatusLabel = t(STATUS_LABEL_KEYS[data.newStatus] || 'status') ;
        return `${t('shipNotifNewStatus')}: ${newStatusLabel}`;
      case 'eta':
        return `${t('shipNotifNewDate')}: ${data.formattedEta}`;
      case 'arrived':
        return `${data.vehicleTitle || data.vin} ${t('shipNotifArrivedSuffix')}`;
      case 'ready':
        return `${data.vehicleTitle || data.vin} ${t('shipNotifReadySuffix')}`;
      default:
        return data.message || t('adm3_8dc0db7663');
    }
  };
