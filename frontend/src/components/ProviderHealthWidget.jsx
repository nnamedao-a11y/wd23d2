    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="text-right text-xs opacity-80">
          <div>{t('r9_orders_label')}: <b className="text-white">{stats.metrics?.totalOrders ?? 0}</b></div>
          <div>{t('r9_completed_label')}: <b className="text-white">{stats.metrics?.completedOrders ?? 0}</b></div>
          {stats.metrics?.responseTimeAvg !== null && stats.metrics?.responseTimeAvg !== undefined && (
            <div>{t('r9_avg_start_label')}: <b className="text-white">{stats.metrics.responseTimeAvg} {t('r9_min_short')}</b></div>
          )}
        </div>
      </div>
      ...
      <div className="mt-3 text-sm font-medium leading-snug" data-testid="provider-health-message">
        {t(`r9_tier_msg_${stats.tier}`) || stats.message || '—'}
      </div>
    </div>
  );
}
