...
// Status configuration with icons and colors — label resolved via t(labelKey) at render time.
const STATUS_CONFIG = {
  deal_created:        { icon: FileCheck,   labelKey: 'shipStatusDealCreated',       color: 'gray' },
  contract_signed:     { icon: FileCheck,   labelKey: 'shipStatusContractSigned',    color: 'blue' },
  deposit_paid:        { icon: CheckCircle, labelKey: 'shipStatusDepositPaid',       color: 'green' },
  lot_paid:            { icon: CheckCircle, labelKey: 'shipStatusLotPaid',           color: 'green' },
  transport_to_port:   { icon: Truck,       labelKey: 'shipStatusTransportToPort',   color: 'yellow' },
  at_origin_port:      { icon: Anchor,      labelKey: 'shipStatusAtOriginPort',      color: 'blue' },
  loaded_on_vessel:    { icon: Ship,        labelKey: 'shipStatusLoadedOnVessel',    color: 'blue' },
  in_transit:          { icon: Ship,        labelKey: 'shipStatusInTransit',         color: 'purple' },
  at_destination_port: { icon: Anchor,      labelKey: 'shipStatusAtDestinationPort', color: 'blue' },
  customs:             { icon: FileCheck,   labelKey: 'shipStatusCustoms',           color: 'orange' },
  ready_for_pickup:    { icon: Package,     labelKey: 'shipStatusReadyForPickup',    color: 'green' },
  delivered:           { icon: CheckCircle, labelKey: 'shipStatusDelivered',         color: 'green' },
  cancelled:           { icon: AlertCircle, labelKey: 'shipStatusCancelled',         color: 'red' },
};
    ...
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        {shipment.containerNumber && (
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase">{t('adm3_df7a6b700b')}</p>
            <p className="font-mono text-sm font-medium">{shipment.containerNumber}</p>
          </div>
        )}
        {shipment.vesselName && (
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase">{t('adm3_ae0d6313ed')}</p>
            <p className="text-sm font-medium">{shipment.vesselName}</p>
          </div>
        )}
        {shipment.eta && (
          <div className="bg-blue-50 rounded-lg p-3">
            <p className="text-xs text-blue-600 uppercase">{t('adm3_eb477b6c14')}</p>
            <p className="text-sm font-medium text-blue-700">
              {new Date(shipment.eta).toLocaleDateString(getLocale())}
            </p>
          </div>
        )}
      </div>
      ...
      {(shipment.originPort || shipment.destinationPort) && (
        <div className="flex items-center gap-4 mb-6 p-4 bg-gradient-to-r from-blue-50 to-green-50 rounded-lg">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">{t('adm3_dcb420e48a')}</p>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">{t('adm3_ddfe8953ba')}</p>
  ...
  if (!events || events.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-500 text-center py-8">{t('adm3_3512d954eb')}</p>
      </div>
    );
  }
        ...
        {events.map((event, index) => (
          ...
          >
            ...
            <div className="flex-1">
              <p className="font-medium text-gray-900">{tSeed(event.title, lang)}</p>
              {event.description && (
                <p className="text-sm text-gray-600 mt-1">{tSeed(event.description, lang)}</p>
              )}
              <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                {event.location && (
                  <span className="flex items-center gap-1">
                    <MapPin className="h-3 w-3" />
                    {tSeed(event.location, lang)}
                  </span>
                )}
                <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" />
                  {new Date(event.eventDate).toLocaleDateString(lang === 'en' ? 'en-US' : lang === 'bg' ? 'bg-BG' : getLocale(), {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric',
                    hour: '2-digit',
