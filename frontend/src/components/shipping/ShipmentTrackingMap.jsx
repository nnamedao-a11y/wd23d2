...
const ShipmentTrackingMap = ({ shipment, liveUpdate }) => {
  ...
  const formatEta = (iso) => {
    ...
    try {
      ...
      return { formatted: d.toLocaleDateString(getLocale(), { day: 'numeric', month: 'long', year: 'numeric' }), days };
    } catch {
      return null;
    }
  };
        ...
        {vessel?.name && (
          <div className="absolute top-4 left-4 bg-slate-900/85 backdrop-blur-md border border-slate-700/80 rounded-xl px-4 py-2.5 shadow-lg">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold">{t('cmp_vessel')}</div>
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
          ...
          <div className="bg-slate-900/85 backdrop-blur-md border border-slate-700/80 rounded-xl shadow-lg px-3 py-2.5 min-w-[200px] space-y-1.5">
            <div className="flex items-center justify-between gap-3 text-[11px]">
              <span className="text-slate-400 flex items-center gap-1"><Gauge size={11} /> {t('cmp_speed_2')}</span>
              <span className="text-white font-mono font-semibold">
                {vesselSpeed != null ? `${Number(vesselSpeed).toFixed(1)} kn` : '—'}
              </span>
            </div>
            <div className="flex items-center justify-between gap-3 text-[11px]">
              <span className="text-slate-400 flex items-center gap-1"><Compass size={11} /> {t('cmp_rate')}</span>
              <span className="text-white font-mono font-semibold">
                {vesselCourse != null ? `${Math.round(vesselCourse)}°` : '—'}
              </span>
            </div>
            <div className="flex items-center justify-between gap-3 text-[11px]">
              <span className="text-slate-400 flex items-center gap-1"><NavigationArrow size={11} /> {t('cmp_position')}</span>
        ...
        <div className="absolute bottom-4 left-4 right-4 md:right-auto md:min-w-[360px] bg-slate-900/85 backdrop-blur-md border border-slate-700/80 rounded-xl shadow-lg px-4 py-3">
          ...
          {etaInfo && (
            <div className="mt-1.5 text-[10px] text-slate-400 flex items-center justify-between">
              <span>{t('cmp_arrival_2')} <b className="text-white">{etaInfo.formatted}</b></span>
              <span>
                {etaInfo.days === 0 ? t('adm3_d7076a6a85') : `${t('r9_in_time_prefix')}${etaInfo.days} ${etaInfo.days === 1 ? t('adm3_48e438a146') : t('adm3_e85d4cee49')}`}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('cmp_route')}</span>
          </div>
          <div className="space-y-1.5 text-sm">
            <div className="flex items-start gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-sky-500 mt-1.5 flex-shrink-0"></div>
              <div className="min-w-0">
                <div className="text-[10px] text-slate-400 uppercase">{t('cmp_shipment')}</div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                <div className="text-[10px] text-slate-400 uppercase">{t('cmp_destination')}</div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('cmp_progress')}</span>
          </div>
          <div className="flex items-baseline gap-1.5 mb-2">
            <span className="text-3xl font-bold text-slate-900">
              {Math.round(progressClamped * 100)}%
            </span>
            <span className="text-xs text-slate-500">{t('cmp_passed')}</span>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">{t('cmp_arrival')}</span>
          </div>
          {etaInfo ? (
            <>
              ...
              {isLive && (
                <div className="text-[10px] text-emerald-600 mt-1 font-medium">
                  {t('cmp_according_to_vessel_data')}
                </div>
              )}
            </>
          ) : (
            <div className="text-sm text-slate-400">{t('cmp_not_defined')}</div>
          )}
        </div>
      </div>
    </div>
