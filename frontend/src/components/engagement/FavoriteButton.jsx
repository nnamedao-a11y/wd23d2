...
export default function FavoriteButton({
  ...
  const requireAuth = useCallback(() => {
    ...
    toast.info(t('cmp_log_in_to_add_to_favorites'), {
      description: t('cmp_redirecting_to_login_page'),
      duration: 2500,
    });
  ...
  const handleClick = useCallback(
    async (e) => {
      ...
      if (!id) {
        toast.error(t('cmp_no_vin_cannot_save'));
        return;
      }
      ...
      try {
        ...
        if (err?.status === 401) {
          ...
          toast.error(err?.message || t('r9_failed_to_update_favs'));
        }
      } finally {
        setBusy(false);
  ...
  if (variant === 'icon') {
    ...
    return (
      <button
        ...
        onClick={handleClick}
        disabled={busy}
        title={isFav ? t('r9_remove_from_favs') : t('r9_add_to_favs')}
        aria-pressed={isFav}
  ...
  return (
    ...
    >
      ...
      />
      {showText && (
        <span>{busy ? '…' : isFav ? t('r9_in_favorites') : t('r9_add_to_favorites_btn')}</span>
      )}
    </button>
  );
}
