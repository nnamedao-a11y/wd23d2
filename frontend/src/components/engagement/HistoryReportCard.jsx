        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-lg">
              {hasIssues ? t('r9_issues_found') : t('r9_clean_history')}
            </h3>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard 
            ...
            label={t('cmp_title')}
            value={data.titleIssues?.length ? t('r9_issues') : t('r9_clean')}
            warning={data.titleIssues?.length > 0}
          />
          <StatCard 
            icon={<Speedometer size={20} />}
            label={t('cmp_odometer')}
            value={data.odometerFlags?.length ? t('r9_flags') : 'OK'}
        ...
        <div className="pt-4 border-t text-sm text-zinc-500">
          <div className="flex justify-between">
            <span>{t('r9_provider')}{report.provider}</span>
            <span>{t('r9_updated')}{new Date(report.createdAt).toLocaleDateString(getLocale())}</span>
          </div>
        </div>
      </div>
    </div>
