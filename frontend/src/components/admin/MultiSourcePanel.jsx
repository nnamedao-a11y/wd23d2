  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        <div>
          <p className="text-[9px] text-[#A1A1AA] uppercase tracking-wide">{t('callsTab')}</p>
          <p className="text-sm font-bold text-[#18181B]">{calls}</p>
        </div>
        <div>
          <p className="text-[9px] text-[#A1A1AA] uppercase tracking-wide">{t('hitsLabel')}</p>
          <p className="text-sm font-bold text-emerald-600">{hits}</p>
        </div>
        <div>
          <p className="text-[9px] text-[#A1A1AA] uppercase tracking-wide">{t('errorsLabel')}</p>
      ...
      <div className="flex items-center justify-between text-[10px] text-[#71717A] mb-3 border-t border-[#F4F4F5] pt-2">
        <span>
          {t('cmp_p50')} <span className="font-mono text-[#18181B]">{info?.latency_p50_ms || 0}ms</span>
        </span>
        <span>
          {t('cmp_p95')} <span className="font-mono text-[#18181B]">{info?.latency_p95_ms || 0}ms</span>
        </span>
        <span>
          {t('cmp_n')}<span className="font-mono text-[#18181B]">{info?.sample_size || 0}</span>
        </span>
      </div>
...
const MultiSourcePanel = () => {
  ...
  const runAaTest = async () => {
    if (!testVin || testVin.length !== 17) {
      toast.error(t('cmp_vin_must_be_17_chars'));
      return;
    }
    setTesting(true);
    setTestRes(null);
    try {
      ...
      setTestRes(r.data);
      if (r.data?.found) toast.success(t('cmp_found_on_auctionauto'));
      else toast.message(t('cmp_not_found_vin_may_have_rotated_out_of_inventory'));
    } catch (e) {
      setTestRes({ error: e?.response?.data?.detail || String(e) });
      toast.error(t('cmp_aa_smoketest_failed'));
    } finally {
      setTesting(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            <h3 className="text-sm font-semibold text-[#18181B]">{t('extensionClients')}</h3>
              ...
              {onlineClients === 0 && clients?.total === 0 && (
                <>
                  ...
                  <span className="text-amber-600">
                    {t('cmp_no_extensions_registered_install_bibi_cars_parser')}
                  </span>
                </>
              )}
            </p>
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-[#18181B]">
            {t('liveAuctionAutoSmokeTest')}
          </h3>
        </div>
        <p className="text-[11px] text-[#71717A] mb-3">{t('callsTab')}<code className="font-mono">/api/ext/auctionauto/test</code> directly,
