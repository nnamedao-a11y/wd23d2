...
const WestMotorsPanel = () => {
  ...
  const cancelRun = async () => {
    ...
    try {
      ...
      toast.success(t('cmp_cancellation_signal_sent'));
    } catch (e) {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      toast.success(t('cmp_settings_saved'));
  ...
  if (!status) {
    return (
      <div className="bg-white rounded-xl border border-[#E4E4E7] p-6 flex items-center gap-3">
        ...
        <span className="text-sm text-[#52525B]">{t('cmp_loading_westmotors_status')}</span>
      </div>
    );
  }
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#71717A]">
              {t('cmp_sitemapdriven_vin_fallback_for_westmotorspl_fires')}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <StatusBadge active={fullSchedulerOn} label={t('fullDaily')} />
          <StatusBadge active={incSchedulerOn} label={t('incrementalHourly')} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
        <Tile icon={Database} label={t('indexedVins')} value={db?.total ?? 0} accent />
        <Tile icon={CheckCircle} label={t('activeStatusGeneric')} value={db?.active ?? 0} />
        <Tile icon={XCircle} label={t('archivedStatus')} value={db?.archived ?? 0} />
        <Tile
          icon={Lightning}
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        ...
        <button
          ...
          title={t('cmp_prefetch_the_mostsearched_vins_from_search_logs')}
        >
        ...
        <div className="overflow-x-auto rounded-xl border border-[#E4E4E7]">
          <table className="w-full text-sm" data-testid="wm-runs-table">
            <thead className="bg-[#FAFAFA] text-[11px] uppercase tracking-wider text-[#71717A]">
              <tr>
                <th className="px-3 py-2 text-left">{t('whenLabel')}</th>
                <th className="px-3 py-2 text-left">{t('kindLabel')}</th>
                <th className="px-3 py-2 text-right">{t('seenLabel')}</th>
                <th className="px-3 py-2 text-right">{t('cmp_new')}</th>
                <th className="px-3 py-2 text-right">{t('updated')}</th>
                <th className="px-3 py-2 text-right">{t('archivedStatus')}</th>
                <th className="px-3 py-2 text-right">{t('errorsLabel')}</th>
                <th className="px-3 py-2 text-right">{t('durationGeneric')}</th>
              </tr>
            </thead>
              ...
              {runs.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-3 py-6 text-center text-[#A1A1AA]">
                    {t('cmp_no_runs_yet_fire_one_with_the_buttons_above')}
                  </td>
                </tr>
              ) : (
                runs.map((r) => (
